...
public enum SwEnum {
    ...
    SwEnum(String pStatus, String pDetail) {
    ...
    public static SwEnum getSW(byte[] pData) {
        ...
        for (SwEnum val : values()) {
            byte[] bArr = val.status;
            if (bArr.length != 1 || pData[pData.length - 2] != bArr[0]) {
                byte b = pData[pData.length - 2];
                byte[] bArr2 = val.status;
                if (b != bArr2[0] || pData[pData.length - 1] != bArr2[1]) {
                }
            }
            return val;
        }
