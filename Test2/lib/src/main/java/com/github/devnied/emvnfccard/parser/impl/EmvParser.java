...
public class EmvParser extends AbstractParser {
    ...
    protected boolean extractPublicData(Application pApplication) throws IOException {
        boolean ret = false;
        ...
        if (ResponseUtils.contains(data, SwEnum.SW_9000, SwEnum.SW_6285)) {
            ...
            if (ret) {
                ...
                this.template.get().getCard().setType(findCardScheme(aid, this.template.get().getCard().getCardNumber()));
    ...
    protected boolean parse(byte[] pSelectResponse, Application pApplication) throws IOException {
        ...
        if (!ResponseUtils.isSucceed(gpo)) {
            ...
            if (pdol == null || !ResponseUtils.isSucceed(gpo)) {
                gpo = this.template.get().getProvider().transceive(new CommandApdu(CommandEnum.READ_RECORD, 1, 12, 0).toBytes());
                if (!ResponseUtils.isSucceed(gpo)) {
                    return false;
                }
            }
    ...
    protected boolean extractCommonsCardData(byte[] pGpo) throws IOException {
        boolean ret = false;
        byte[] data = TlvUtil.getValue(pGpo, EmvTags.RESPONSE_MESSAGE_TEMPLATE_1);
        if (data != null) {
            data = ArrayUtils.subarray(data, 2, data.length);
        } else {
            ret = extractTrackData(this.template.get().getCard(), pGpo);
            if (!ret) {
        ...
        if (data != null) {
            ...
            for (Afl afl : listAfl) {
                for (int index = afl.getFirstRecord(); index <= afl.getLastRecord(); index++) {
                    byte[] info = this.template.get().getProvider().transceive(new CommandApdu(CommandEnum.READ_RECORD, index, (afl.getSfi() << 3) | 4, 0).toBytes());
                    if (ResponseUtils.isSucceed(info)) {
                        extractCardHolderName(info);
                        if (extractTrackData(this.template.get().getCard(), info)) {
                            return true;
                        }
                    }
                }
    ...
    protected byte[] getGetProcessingOptions(byte[] pPdol) throws IOException {
        ...
        try {
            ...
            if (list != null) {
                for (TagAndLength tl : list) {
                    out.write(this.template.get().getTerminal().constructValue(tl));
                }
            }
        } catch (IOException ioe) {
            LOGGER.error("Construct GPO Command:" + ioe.getMessage(), (Throwable) ioe);
        }
        return this.template.get().getProvider().transceive(new CommandApdu(CommandEnum.GPO, out.toByteArray(), 0).toBytes());
    }

    protected boolean extractTrackData(EmvCard pEmvCard, byte[] pData) {
        this.template.get().getCard().setTrack1(TrackUtils.extractTrack1Data(TlvUtil.getValue(pData, EmvTags.TRACK1_DATA)));
        this.template.get().getCard().setTrack2(TrackUtils.extractTrack2EquivalentData(TlvUtil.getValue(pData, EmvTags.TRACK_2_EQV_DATA, EmvTags.TRACK2_DATA)));
        return (pEmvCard.getTrack1() == null && pEmvCard.getTrack2() == null) ? false : true;
    }
}
