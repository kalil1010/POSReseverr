...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f57mX = new float[numPoints];
        this.f58mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f57mX[i] = position[0];
            this.f58mY[i] = position[1];
        }
        if (Math.abs(this.f57mX[0]) > 1.0E-5d || Math.abs(this.f58mY[0]) > 1.0E-5d || Math.abs(this.f57mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f58mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f57mX[0] + "," + this.f58mY[0] + " end:" + this.f57mX[numPoints - 1] + "," + this.f58mY[numPoints - 1]);
        }
        float prevX = 0.0f;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f57mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f57mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
